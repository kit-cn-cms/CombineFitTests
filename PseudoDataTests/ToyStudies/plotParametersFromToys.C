...
void plotTree(TTree *tree_, std::string whichfit, std::string selectString){
	...
	for (int iobj=0;iobj<nBranches;iobj++){
		...
		if (TString(name).Contains("BDTbin") or TString(name).Contains("n_exp") or TString(name).Contains("_In") ){
...
// 		for(int i=0;i<bH->GetNbinsX();i++){std::cout<<bH->GetBinContent(i)<<std::endl;}
		...
		if ( isFitted ){
			...
			TH2D *h2d_corr = (TH2D*)gROOT->FindObject(Form("%s_%s_2d",name,tree_->GetName()));
			h2d_corr->SetStats(1);
			h2d_corr->Draw();
		...
		if (TString(nameTwo).Contains("BDTbin") or TString(nameTwo).Contains("n_exp") or TString(nameTwo).Contains("_In") or !(TString(nameTwo).Contains("CMS")) or doDrawFullCorrelation==0){
			  continue;
			}
...
// 			std::cout<<"forms 2D "<<drawInput1<<" "<<drawInput2<<std::endl;
	...
	for(unsigned int mip=0;mip<myParamNames.size();mip++){
          if (myParamNames.at(mip).Contains("BDTbin")){continue;}
...
void plotParametersFromToys(std::string inputFile, std::string dataFits="", std::string workspace="", std::string selectString="",bool DrawFullCorrelation=0){
	...
	gStyle->SetPalette(1,0);
        if(DrawFullCorrelation==1){doDrawFullCorrelation=1;}
