        ...
        if isinstance(data_obs, ROOT.TGraphAsymmErrors):
            ...
            for i in range(data_obs.GetN()):
                ...
                if not i in data_obs_dic[cat]:
                    uid = ROOT.TUUID()
                    data_obs_dic[cat][i] = ROOT.TH1D("data_obs_distr_"+cat+"_"+str(i)+"_"+uid.AsString(), ";integral;frequency", 400, val-10*(val)**(1./2.),val+30*(val)**(1./2.))
    ...
    for path in sorted(glob.glob(path1)):
        if os.path.exists(os.path.abspath(path)):
            ...
            for infile in filesToCheck:
                ...
                if os.path.exists(pathToLoad) and os.path.exists(siblingPath):
