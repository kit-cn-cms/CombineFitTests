...
#___________________________________________________
	...
	if [[ -d "$outputPath" ]]; then
		...
		if [[ -f $toyDatacard ]]; then
			combineCmd="combine -M GenerateOnly -m 125 --saveToys -t $numberOfToysPerExperiment -n _$((numberOfToysPerExperiment))toys_sig$signalStrength --expectSignal $signalStrength -s $((randomseed)) --toysNoSystematics $toyDatacard"
			echo "$combineCmd"
			eval $combineCmd
