...
if [[ -d $outputPath ]]; then
	...
	for (( i = $((lowerBound+1)); i <= $upperBound; i++ )); do
		...
		if [[ -d PseudoExperiment$i ]]; then
			...
			eval "/nfs/dust/cms/user/firin/bachelor/CombineFitTests/PseudoDataTests/scripts/toy_tests/bla_asimov_2/temp/generateToysAndFits.sh $signalStrength $((i+200)) $outputPath/PseudoExperiment$i $numberOfToysPerExperiment"
